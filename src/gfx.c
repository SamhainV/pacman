...
void fastflip_screen(SDL_Surface *dest, unsigned char *source)
{
  ...
  //memcpy32(dest->pixels, source, 16000);
...
/*void putico(int x, int y, Uint32 *source, Uint32 *dest, int max_x, int max_y)
{
  ...
  for (i = 0; i < max_y; i++)
    {
      memcpy(tmpdest+x+(y<<8)+(y<<6), tmpsrc, max_x);
      tmpsrc+=max_x;
      tmpdest+=320;
    }
}*/
...
void TimerStop(void)
{
  setitimer(ITIMER_REAL,&oldtimer,&oldtimer);
  sigaction(SIGALRM,&oldaction,&oldaction);
}
